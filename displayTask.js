      ...
      for(let taskIndex = 0; taskIndex < employee.Task.length; taskIndex++){
         for(let k = 1; k < myTable.rows[0].cells.length; k++) {
           ...
           if(myTable.rows[0].cells[k].innerHTML == employee.Task[taskIndex]["Task Start Date"]) {
             colFill = true;
             taskStart = myTable.rows[0].cells[k].cellIndex;
           }
           if(myTable.rows[0].cells[k].innerHTML == employee.Task[taskIndex]["Task End Date"]) {
             taskEnd = myTable.rows[0].cells[k].cellIndex;

             taskMid = taskEnd - taskStart;
             
             colFill = false;
           }
           if(colFill == true) {
             // add task name to the cell


             myTable.rows[j+1].cells[k].innerHTML += '<div style="background-color:' + color + '">' + employee.Task[taskIndex]["Task Name"] + '</div>';
           }
